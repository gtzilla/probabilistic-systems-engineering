...
Each eligible document MUST map to exactly one project via Folder Spec configuration. Folder name MUST NOT be used for project derivation.  
 If project normalization fails, outcome MUST be `SKIPPED_METADATA`.
...
* **Explicitly unsupported**: the API explicitly indicates tab content is unavailable/unsupported for this document type/account.
...
* `artifacts/<project_norm>/<source_id>/meta.json`
...
* Tab boundaries MUST be represented deterministically in `doc.md`.
...
Idempotence applies to identical `source_snapshot` (the normalized structural representation derived from the Source Model).  
 Given identical `source_snapshot`:
...
* If a tie remains after index and array order, tie-break by `tabProperties.tabId` lexical ascending.
...
* `# [TAB] <title> (<tab_id>)`  
* If `<title>` is empty or unavailable: `# [TAB] (<tab_id>)`

`<tab_id>` MUST be `tabProperties.tabId`.  
 If `<tab_id>` is missing for any tab, outcome MUST be `FAILED_OPERATIONAL`.
...
* The `<title>` segment MUST be wrapped in backticks.  
* Any backtick characters inside the title MUST be escaped by doubling them.
...
On successful export of a Source Document, exporter MAY rewrite an existing Managed Unit for the same `source_id` to the current schema\_version ("2.6.3") by overwriting `doc.md` and `meta.json` per this contract.  
...
CI runs MUST execute with deletions disabled.  
 CI MUST NOT set `ARTIFACT_SYNC_ALLOW_DELETIONS=true` except in a deletion-specific test job explicitly designed to validate deletion behavior.
...
* At least one multi-tab document fixture:  
  * expected `doc.md` includes at least two tab boundary markers  
  * expected `doc.md` includes content from more than one tab  
